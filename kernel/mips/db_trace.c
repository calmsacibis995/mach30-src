...
void
db_mips_stack_trace(count, stackp, the_pc, the_ra, flags, kstackp, thread)
	...
	thread_t	thread;
{
	...
	if (!_setjmp(db_recover = &db_jmpbuf)) {
		...
		s = db_get_value(cproc + db_cproc_state_offset, 4, FALSE, task);
		w = db_get_value(cproc + db_cproc_wait_offset, 4, FALSE, task);
		n = db_get_value(cproc + db_cproc_next_offset, 4, FALSE, task);
		c = db_get_value(cproc + db_cproc_context_offset, 4, FALSE, task);
		cth = db_get_value(cproc + db_cproc_incarnation_offset, 4, FALSE, task);
		...
		if ((s != 0) && (c != 0)) {
			...
			pc = db_get_value(c + db_cproc_pc_offset, 4, FALSE, task);
...
db_all_cprocs()
{
	...
	while (next) {
		...
		db_trace_cproc(cproc, current_thread());
	}
